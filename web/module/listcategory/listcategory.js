...
angular.module('listcategory', [])
    .controller('listcategory', function($window,$rootScope,$scope,$state,$stateParams,Project) {
        ...
        {
            var value = prompt("请输入分类名称",item.categoryName);
            event.stopPropagation();
            if (value.length == null){
                alert("请输入分类名称.");
                return;
            }
            Project.updateProjectCategory($scope.proj.projectName,$scope.proj.projectName,item.categoryName,value).then(function(response)
            {
                console.log('' + response);
                if (response.errCode == '0'){
                    $scope.refresh();
